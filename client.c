...
int upload_file(int server_fd, char* command){
	...
	char* message = "File is uploaded to server successfully!\n";
...
int download_file(int server_fd, char* command){
	...
	char* message = "File is downloaded from server successfully!\n";
	write(2, message, strlen(message));
	return 1;
}
...
void send_broadcast(int udp_port, char* message){
	...
	char* msg = "broadcasting filename!\n";
...
void initial_heartbeat(int heartbeat_port){
	...
	heartbeat_address.sin_port = htons(heartbeat_port);
...
int send_file(int destination_fd, char* file_name){
	...
	if(read_length < 0){
    	char* error_message = "read error from file for sending!\n";
		write(2, error_message, strlen(error_message));
		return 0;
	}
...
int receive_file(int source_fd, char* file_name){
    ...
    strcat(dest,file_name);
	if ((file_fd = open(dest, O_RDWR | O_CREAT | O_TRUNC, S_IRWXU)) < 0){
		char* error_message = "create file error!\n";
		write(2, error_message, strlen(error_message));
		return 0;
	}
...
int main(int argc, char const *argv[]){
    char* msg = "enter your directory:";
    write(2,msg,strlen(msg));
    int size_dir = read(0,path,100);
    path[size_dir-1]='\0';
    mkdir(path,1);
	...
	// BroadCast
	initial_broadcast(client_broadcast_port);

	// HeartBeat
	initial_heartbeat(heartbeat_port);
	...
	while (1)
	{
		...
		if (can_send_broadcast && find_with_broadcast){
			...
			write(2, sending_message, strlen(sending_message));
		...
		if (data_is_ready && (read_size = read(0, command, 100)) < 0){
			...
			if (memmem(command, read_size, "download", 8) == command){
				...
				if (server_is_alive)
					...
					char* error_message = "server is down for uploading.\n";
					write(1, error_message, strlen(error_message));
				}
				...
				char* message = "Invalid command. Try again!(enter download filename or upload filename)\n";
				write(2, message, strlen(message));
			}
		}
